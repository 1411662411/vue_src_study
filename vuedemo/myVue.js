...
MyVue.prototype._init = function (options) {
    const vm = this
    vm.$options = options      //配置项添加到vue实例中
    vm._renderProxy = vm       // ??? 
    initState(vm)             //初始化数据监控与订阅者  
    vm.$mount()               //加载到页面中     
}
//$mount函数合二为一,删除el有关内容
MyVue.prototype.$mount = function () {
    function render(){ with(this){ return `我的名字: ${name}, 我的年龄: ${age}` }}   //重新定义一个render函数
    this.$options.render = render                                                   
    mountComponent(this)                                                           //挂在元素
} 
//实际在renderMixin中被注册
...
class Watcher {
    constructor(vm, expOrFn, cb, options, isRenderWatcher) {
        this.vm = vm
        vm._watchers.push(this)                             //把给vue实例 加上watcher   
        this.cb = cb
